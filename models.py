...
class BertGRU:
    def __init__(self, GRU_UNITS=128, dropout=0.2):
        ...
        self.dropout = dropout
        self.DENSE_HIDDEN_UNITS = 4 * GRU_UNITS  # number of units in
    ...
    def build_model(self, embedding_matrix, labels, out=1):
        ...
        hidden_x = concatenate(
            [GlobalMaxPooling1D()(x), GlobalAveragePooling1D()(x)], axis=1
        )
        hidden_y = concatenate(
            [GlobalMaxPooling1D()(y), GlobalAveragePooling1D()(y)], axis=1
        )
        hidden_z = concatenate(
            [GlobalMaxPooling1D()(z), GlobalAveragePooling1D()(z)], axis=1
        )

        # Concatenate the global pooling outputs from each GRU layer
        hidden = concatenate([hidden_x, hidden_y, hidden_z], axis=1)
        hidden = Dense(self.DENSE_HIDDEN_UNITS, activation="relu")(
            hidden
        )  # dense hidden layer
        logits = Dense(labels * out, activation="softmax")(
            hidden
        )  # output layer with softmax activation

        if out != 1:
            logits = Reshape((out, labels))(logits)  # Reshape for multi targets.
